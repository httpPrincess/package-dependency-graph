...
def clean_graph(graph):
    ...
    try:
        ...
        logger.error('Problem cleaning up graph %r', error)


def get_graph(cleanup=False):
    uri = getenv('NEO4J_URI', 'http://neo4j:neo@localhost:7474/db/data/')
    logger.info('Connecting to graph at: %s', uri)
    graph = Graph(uri)
    if cleanup:
        clean_graph(graph)
    try:
...
def process_package(graph, package):
    ...
    for dep in deps.split(','):
        ...
        logger.info('(%s)-[:DEPENDS_ON]->(%s)', package['name'], dep_name)
        graph.create_unique(Path(p, 'DEPENDS_ON', dep))


def parse_package_list(input):
    packages = []
    for l in input:
        try:
            packages.append(json.loads(l))
        except Exception as inst:
            logger.error('Error processing %s %s', l, inst)
    return packages
...
if __name__ == "__main__":
    ...
    if path.isfile(fname):
        logger.info('Retrieving package list from %s', fname)
        packages = get_packages_from_file(fname)
    else:
        logger.info('Generating package list with dpkg-query')
        packages = generate_package_list()

    if packages is None or len(packages) == 0:
        logger.error('Unable to create package list')
        exit(-1)

    logger.info('Got %d packages' % len(packages))
